...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Existing bookings not yet synced to Google (no gcal_event_id). If column missing, sync all non-cancelled.
    let bookings: { id: string }[] = [];
    const { data: withGcal, error: e1 } = await supabase
      .from("bookings")
      .select("id")
    ...
    if (e1) {
      ...
      bookings = all.map((b) => ({ id: b.id }));
    } else {
      bookings = (withGcal ?? []).map((b) => ({ id: b.id }));
    }
    if (!bookings.length) {
      return new Response(JSON.stringify({ synced: 0, message: "No bookings to sync" }), {
        status: 200,
