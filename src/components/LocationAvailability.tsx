...
function buildScheduleFromData(data: { day_of_week: number; start_time: string; end_time: string; id?: string }[]): WeekSchedule {
...
export function LocationAvailability({ locationId, locationName }: LocationAvailabilityProps) {
  ...
  const { data: availability = [] } = useQuery({
    queryKey: ["location-availability", locationId],
    queryFn: async () => {
  ...
  useEffect(() => {
    // Only overwrite local schedule from server when user has no unsaved changes (stops toggles "closing back" on refetch)
    if (!hasChanges) {
  ...
  const saveSchedule = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["location-availability", locationId] });
      setHasChanges(false);
                  ...
                  {daySchedule.slots.map((slot, slotIdx) => {
                    ...
                    return (
                      <div key={slotIdx} className="flex items-center gap-2 flex-wrap">
                        ...
                        >
                          <SelectTrigger className={cn("w-[110px] h-8 text-xs", error && "border-destructive")}>
